...
public abstract class StringRenderOutputMixin implements StringRenderOutputAccessor {
    ...
    private void renderBadgeShadowOrOutline(int i, Style style, int j, CallbackInfoReturnable<Boolean> cir, FontSet fontSet, GlyphInfo glyphInfo, BakedGlyph bakedGlyph, boolean bl, float g, float h, float l, float f, TextColor textColor, float m, float n, VertexConsumer vertexConsumer) {// prefer 1-3-4-5
        ...
        if (!(this.dropShadow && isInBadge) && this.mode != Font.DisplayMode.SEE_THROUGH && bgRenderMode != 0 && bgRenderMode != 2) {
            ...
            glyphColor = ColorHelper.applyLuminance(glyphColor, this.getDim());
            for (int dx = 0; dx <= (style.isBold() ? 1 : 0); dx++)
                bakedGlyph.render(style.isItalic(), this.x + n + dx * glyphInfo.getBoldOffset(), this.y + n, this.pose, vertexConsumer, glyphColor.getRed() / 255.0f, glyphColor.getGreen() / 255.0f, glyphColor.getBlue() / 255.0f, f, this.packedLightCoords);
        }
    ...
    private void renderForegroundText(Font instance, BakedGlyph glyph, boolean bold, boolean italic, float boldOffset, float x, float y, Matrix4f matrix, VertexConsumer buffer, float red, float green, float blue, float alpha, int packedLight) {
        ...
        var isOutline = ((BakedGlyphAccessor) glyph).isOutline(); // prev glyph render state
        if ((isInBadge || isOutline) && this.dropShadow) return; // prefer 0-2

        var overlayBuffer = this.bufferSource.getBuffer(glyph.renderType(this.mode == Font.DisplayMode.SEE_THROUGH ? this.mode : (isInBadge || isOutline) ? Font.DisplayMode.POLYGON_OFFSET : Font.DisplayMode.NORMAL));
        if (isOutline)
            ((BakedGlyphAccessor) glyph).setRenderMode(((BakedGlyphAccessor) glyph).getRenderMode() - 1);
    ...
    private void invokeBadgeRenderer(int backgroundColor, float x, CallbackInfoReturnable<Float> cir) {
        ...
        for (var buffer : this.badgeBuffer)
            BadgeRenderer.render(buffer.getBadge(), buffer.getX0() + shadowOffset, buffer.getX1() + shadowOffset + 1, this.y - 1 + shadowOffset, this.y + 8 + shadowOffset, depth, this.a, this.getDim(), this.pose, this.bufferSource, this.mode, this.packedLightCoords);
    ...
    private float getDim() {
        return this.dropShadow ? 0.45f : 1.0f;
    }
